...
pub fn handler(ctx: Context<BuyProductSpl>, amount_kg: u64) -> Result<()> {
    require!(amount_kg > 0, UmarketError::InvalidAmount);

    let product = &mut ctx.accounts.product;
    require!(
        matches!(product.payment_mode, PaymentMode::Spl),
    ...
    // Transfer SPL tokens from buyer → escrow token account (PDA-owned)
...
pub struct BuyProductSpl<'info> {
    ...
    /// Escrow-owned token account to hold SPL payment
    #[account(
        init,
        payer = buyer,
